                ...
                if p['receipt'] and transmitter:
                    ...
                    if p['receipt']['signal'] > rssi_lim:
                        ...
                        if transmitter not in bad_neighbors:
                            bad_neighbors[transmitter] = dict(rssi=0, snr=0)
                ...
                for w in p['witnesses']:
                    ...
                    if w['signal'] > rssi_lim:
                        ...
                        if p['challengee'] not in bad_neighbors:
                            bad_neighbors[p['challengee']] = dict(rssi=0, snr=0)
...
def poc_reliability(hotspot, challenges):
        ...
        for p in chal['path'][:-1][::-1]:
            if pnext_pass or p['witnesses'] or p['receipt']:
                if pnext['challengee'] == haddr:
                    if p['challengee'] not in results_tx:
                        results_tx[p['challengee']] = [0, 0]
                    results_tx[p['challengee']][0 if pnext_pass else 1] += 1
                if p['challengee'] == haddr:
                    if pnext['challengee'] not in results_rx:
                        results_rx[pnext['challengee']] = [0, 0]
                    results_rx[pnext['challengee']][0 if pnext_pass else 1] += 1
                pnext_pass = True
            pnext = p
    ...
    def summary_table(results, is_tx=False):
        ...
        if is_tx:
            ...
            print(f"{'to receiving hotspot':30} | {'dist km'} | {'heading'} | pass/ttl | pass % |")
        else:
            print(f"PoC hops to: {hotspot['name']}")
            print(f"{'from transmitting hotspot':30} | {'dist km'} | {'heading'} | pass/ttl | pass % |")
        print("-" * 72)
    ...
    summary_table(results_tx, is_tx=True)
    print()
    print()
    summary_table(results_rx, is_tx=False)
